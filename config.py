...
品質の主軸は「必要な記事を拾い上げる」包含キーワード（医療×IT・医療DX 等）。
任意の拒否語は環境変数 EXCLUDE_EXTRA_KEYWORDS のみ（デフォルトでは拒否リストは空）。
...
# そのまま通す。EXCLUDE_EXTRA_KEYWORDS を設定している場合は filter の Stage 1 で照合される。
...
# =============================================================================
# 任意の拒否キーワード（filter Stage 1。デフォルトは空＝拾い上げ中心）
# =============================================================================
# 本番の取りこぼし・ノイズは MEDICAL / IT / MEDICAL_DX / 優先Googleの包含条件で担保する。
# 運用で明示的に落としたい語だけ EXCLUDE_EXTRA_KEYWORDS（カンマ区切り）に書く。
EXCLUDE_KEYWORDS: list[str] = []

# .env で追加の除外キーワードを渡せるようにする（カンマ区切り）
